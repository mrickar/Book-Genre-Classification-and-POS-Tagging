...
def creatdict(sentence,index,pos):	#pos=="" <-> featuresofword  else, relative pos (str) is pos
	...
	feature_set3_prev_word=dict()

	if index == 0: # is first element in sentence then prev is <s>
		pos = "_prev"
		word="<s>"
		wordlow=word.lower()
		word_root = stemmer.stem(wordlow)
		...
		feature_set3_prev_word= {
			...
			"stop_word"+pos:wordlow in english_stopwords,                   # is stop word
			"punc?"+pos:wordlow in string.punctuation,                      # is punctuation?
			"is_first"+pos:False,                                           # is first word in sentence
			"is_last"+pos:False,                                            # is last word in sentence
		}
	else:
		pos = "_prev"
		...
		feature_set3_prev_word= {
			...
			"stop_word"+pos:wordlow in english_stopwords,                   # is stop word
			"punc?"+pos:wordlow in string.punctuation,                      # is punctuation?
			"is_first"+pos:index-1==0,                                      # is first word in sentence
			"is_last"+pos:index-1==sentence_len-1,                          # is last word in sentence
		}
	feature_set3.update(feature_set3_prev_word)
	feature_set3_next_word=dict()
 
	if  index == (sentence_len-1) : # is last element in sentence then next is <\s>
		pos = "_next"
		word="<\s>"
		wordlow=word.lower()
		word_root = stemmer.stem(wordlow)
	...
	feature_set3.update(feature_set3_next_word)
